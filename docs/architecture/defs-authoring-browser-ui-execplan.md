...
- [ ] Implement frontend HTTP wrappers and page state for defs authoring endpoints, including draft validation, draft creation, draft listing/detail, publish, reload, and description generation.
- [ ] Add browser UI controls on the Defs page for creating and publishing runtime profiles and task types, reusing the existing page-shell, dialog, and state patterns already used by the Tasks page.
