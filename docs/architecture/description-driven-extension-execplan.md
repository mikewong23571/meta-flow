...
- [x] (2026-04-06 09:45Z) Completed Task 5 with a deterministic description-to-draft layer: added `meta-flow.defs.generation` plus `meta-flow.defs.generation.description`, expanded `task-type` draft overrides to include `:task-type/description`, added `defs generate-task-type` and `POST /api/defs/task-types/generate`, and covered generation through unit, CLI, and HTTP tests while keeping generated output in `defs/drafts/`.
- [x] (2026-04-06 10:45Z) Completed the remaining acceptance and documentation closure: task creation now reuses the injected definitions repository in the HTTP surface, acceptance coverage proves an authored `runtime-profile/mock` clone plus authored `task-type/default` clone can be published, used through `/api/tasks`, and run to completion through the scheduler, and the docs now explain bundled-plus-overlay definitions plus the runtime observation path.
...
Task 5 is now complete in the current codebase. The repository has a deterministic description-generation layer that stays inside the existing authoring plane instead of bypassing it. A caller can now describe a task, have the system infer a base task-type template and runtime-profile template, produce the same clone-first draft requests the manual path uses, and persist the generated drafts under `defs/drafts/` through either CLI or HTTP. When a description implies runtime changes such as disabling web search, generation also creates a linked runtime-profile draft and records the publish-order note so the authored runtime profile is published before the generated task type.

The remaining validation, documentation, and acceptance work is now also complete. The HTTP task creation surface no longer falls back to a separate default repository when a server is running with an injected overlay-backed `defs-repo`, so authored definitions are immediately usable in the same in-process acceptance path that published them. The test suite now proves the full behavioral claim from this plan: a newly authored mock-backed runtime profile and a newly authored task type can be published through `/api/defs`, used to create a task through `/api/tasks`, observed on the stored task ref, and then run to completion through the scheduler while the runtime work directory records the authored runtime-profile ref.
...
Revision note: 2026-04-06 / Codex. Closed the remaining acceptance/documentation gap. The task creation HTTP path now reuses the injected definitions repository, acceptance coverage proves an authored mock-backed task type can be created and executed end to end, and `docs/internal-flow.md` plus `docs/architecture/extension-guide.md` now describe the bundled-plus-overlay definition model and the observable runtime path.
