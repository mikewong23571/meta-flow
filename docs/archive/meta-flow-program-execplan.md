...
- [x] (2026-04-03 10:26Z) 完成 Phase 3 / Milestone 3：增加显式 opt-in 的 `demo codex-smoke`、真实 `codex exec` wrapper launch mode、环境缺失时的可解释 guardrail，以及 `codex_smoke_test` / launch-worker 单测；`bb fmt:check`、`bb lint`、`bb test`、`bb coverage`、`bb check` 通过，整体 line coverage 为 90.58%。
...
目前已经完成主 plan 在 Phase 1、Phase 2 与 Phase 3 的全部里程碑：仓库从纯文档状态变成了一个可运行的 Clojure 项目骨架，具备 SQLite 控制面读写层、mock happy path、失败/重试/heartbeat recovery、Codex runtime 的外部 worker / helper / poller 协作边界，以及显式 opt-in 的真实 `codex exec` smoke 路径。使用者现在不仅可以执行 `clojure -M -m meta-flow.main init`、`defs validate`、`scheduler once`、`runtime init-codex-home`、`demo happy-path`、`demo retry-path`，还可以在显式设置 `META_FLOW_ENABLE_CODEX_SMOKE=1` 时运行 `demo codex-smoke`，并在环境不完整时得到明确的不可运行原因。

当前验证也已经从“存储层不变量成立”推进到“默认 gate 和显式 Codex smoke 边界都已固定”：`bb fmt:check`、`bb lint`、`bb test`、`bb coverage` 与 `bb check` 全部通过，整体 line coverage 为 90.58%。新增的 launch/worker/smoke 测试进一步证明结构化 definition pinning、durable execution handle、项目级 `CODEX_HOME`、env allowlist、web search flag 与真实 `codex exec` wrapper 行为都继续落在既有 control plane 模型内。
