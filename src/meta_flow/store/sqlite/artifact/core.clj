...
(declare find-artifact)

(defn- existing-artifact-for-run
  [db-path run-id artifact-id]
  (when-let [existing-artifact (find-artifact db-path artifact-id)]
    (if (= run-id (:artifact/run-id existing-artifact))
      existing-artifact
      (throw (ex-info "Artifact id already belongs to another run"
                      {:artifact/id artifact-id
                       :artifact/run-id (:artifact/run-id existing-artifact)
                       :requested-run-id run-id})))))
...
(defn attach-artifact!
  ...
  (let [_ (shared/require-matching-value! artifact :artifact/run-id run-id)
        ...
        artifact-id (shared/require-key! artifact :artifact/id)
        root-path (or (:artifact/root-path artifact) (:artifact/location artifact))]
    (or (existing-artifact-for-run db-path run-id artifact-id)
        (try
          ...
          (catch java.sql.SQLException throwable
            (if-let [existing-artifact (existing-artifact-for-run db-path run-id artifact-id)]
              existing-artifact
              (throw throwable)))))))
