...
(deftest definition-links-build-indexes-and-reject-broken-references
  (let [definitions (loaded-definitions)
    ...
    (is (contains? (:task-fsms indexes) [:task-fsm/default 3]))
    (is (contains? (:task-fsms indexes) [:task-fsm/cve-investigation 3]))
    (is (contains? (:resource-policies indexes) [:resource-policy/default 3]))
    (is (contains? (:resource-policies indexes) [:resource-policy/serial-cve 3]))
    (is (thrown-with-msg? clojure.lang.ExceptionInfo
