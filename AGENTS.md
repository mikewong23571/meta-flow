...
Application code lives in `src/meta_flow/`. Top-level entry and shared namespaces include `main.clj`, `cli.clj`, `db.clj`, `sql.clj`, `schema.clj`, and the public scheduler facade in `scheduler.clj`; CLI-specific flows are further split under `cli/inspect.clj`. Control-plane primitives live under `control/`, with event ingestion, event constants, FSM helpers, projection queries, and snapshot helpers under `control/projection/`. Workflow definition loading and metadata live under `defs/`, where `loader.clj` fronts `protocol.clj`, `source.clj`, `index.clj`, `validation.clj`, and `repository.clj`. Scheduler internals are split by responsibility under `scheduler/`, including step execution, runtime orchestration, retry policy, shared helpers, dispatch logic in `scheduler/dispatch/core.clj`, and timeout handling in `scheduler/runtime/timeout.clj`. Runtime adapters live under `runtime/`; shared contracts stay in `runtime/protocol.clj` and `runtime/registry.clj`, the mock adapter is split across `runtime/mock.clj` plus `runtime/mock/{fs,events,execution}.clj`, and the Codex adapter is split across `runtime/codex.clj` plus supporting namespaces for filesystem setup, event emission, process launch/state, worker APIs, execution dispatch, launch support, and Codex home management. Storage code lives under `store/`; the protocol is in `store/protocol.clj`, and the SQLite implementation keeps its facade in `store/sqlite.clj` with shared helpers plus task, run, run-data, lease, and artifact persistence separated under `store/sqlite/`, including subareas for `run/`, `lease/`, and `artifact/`. Lint and governance code lives under `lint/`, with file-length checks in `lint/file_length.clj`, structural governance in `lint/check/`, and coverage governance in `lint/coverage/`. Service-layer validation helpers live in `service/validation.clj`. Tests live in `test/meta_flow/`, organized by subsystem under `arch/`, `cli/`, `control/`, `defs/`, `lint/`, `runtime/`, `scheduler/`, and `store/`; scheduler fixtures live under `test/meta_flow/scheduler/support/`, and runtime coverage is further split across Codex launch, worker API, and lower-level unit suites. Repository data and runtime inputs live under `resources/meta_flow/`: workflow definitions in `defs/*.edn`, SQL migrations in `sql/*.sql`, prompt assets in `prompts/`, and bundled Codex home/config fixtures under `codex_home/`. Current repository documentation lives in `docs/`; archived historical material under `docs/archive/` is not part of the active reference set and does not need to be consulted when updating this file.
